...
ext.onReady(function() {
    var i = setInterval(function() {
        if (!IsLoadingData(d) && ext.isReady) {
            ...
            t.activate(0);
            var b = $(d).find('.x-tab-panel-body')[0];
            ...
            if ("{心脏节律}" == "0") {
                ext.getCmp($(field).contents().find('input.x-form-radio[value = "是"]')[0].id).setValue("是");
            } else {
                ext.getCmp($(field).contents().find('input.x-form-radio[value = "否"]')[0].id).setValue("否");
            ...
            if ("{肝脾}" == "0") {
                ext.getCmp($(field).contents().find('input.x-form-radio[value = "未触及"]')[0].id).setValue("未触及");
            } else {
                ext.getCmp($(field).contents().find('input.x-form-radio[value = "触及"]')[0].id).setValue("触及");
